...
ok( $rr1->covered('a.c.x.w.example'), 'B.1(1):	NSEC3 covers "next closer" name (c.x.w.example.)' );	# Name Error
...
ok( !$rr2->covered('a.c.x.w.example'), 'B.1(2):	NSEC3 matches closest encloser (x.w.example.)' );
...
ok( $rr3->covered('*.x.w.example'), 'B.1(3):	NSEC3 covers wildcard at closest encloser (*.x.w.example.)' );
...
ok( !$rr4->covered('ns1.example'), 'B.2:	NSEC3 matches QNAME (ns1.example.) proving MX and CNAME absent' )
		;						# No Data Error
...
ok( !$rr5->covered('y.w.example'), 'B.2.1:	NSEC3 matches QNAME (y.w.example.) proving A absent' )
...
ok( $rr6->covered('mc.c.example'), 'B.3(1):	NSEC3 covers "next closer" name (c.example.)' )
...
ok( !$rr7->covered('mc.c.example'), 'B.3(2):	NSEC3 matches closest provable encloser (example.)' );
...
ok( $rr8->covered('a.z.w.example'), 'B.4:	NSEC3 covers "next closer" name (z.w.example.)' );    # Wildcard Expansion
...
ok( !$rr9->covered('a.z.w.example'), 'B.5(1):	NSEC3 matches closest encloser (w.example.)' );	  # Wildcard No Data Error

my $rr10 = Net::DNS::RR->new(
	"q04jkcevqvmu85r014c7dkba38o0ji5r.example. NSEC3 1 1 12 aabbccdd (
	r53bq7cc2uvmubfu5ocmm6pers9tk9en A RRSIG )"
	);
ok( $rr10->covered('a.z.w.example'), 'B.5(2):	NSEC3 covers "next closer" name (z.w.example.)' );
...
ok( !$rr11->covered('*.w.example'), 'B.5(3):	NSEC3 matches wildcard at closest encloser (*.w.example)' );
...
ok( !$rr12->covered('example'), 'B.6:	NSEC3 matches QNAME (example.) DS type bit not set' )
...
ok( !$rr13->covered('.'), 'ancestor name not covered (.)' );	# too few matching labels

my $rr14 = Net::DNS::RR->new(
	"q04jkcevqvmu85r014c7dkba38o0ji5r.example. NSEC3 1 1 12 aabbccdd (
	53bq7cc2uvmubfu5ocmm6pers9tk9en A RRSIG )"
	);
ok( !$rr14->covered('unrelated.name'), 'name out of zone not covered (unrelated.name.)' );    # non-matching label

my $rr15 = Net::DNS::RR->new(
	"0p9mhaveqvm6t7vbl5lop2u3t2rp3tom.example. NSEC3 1 1 12 aabbccdd (
	2t7b4g4vsa5smi47k61mv5bv1a22bojr )"
	);
ok( !$rr15->covered('example'), 'owner name not covered (example.)' );

my $rr16 = Net::DNS::RR->new(
	"0p9mhaveqvm6t7vbl5lop2u3t2rp3tom.example. NSEC3 1 1 12 aabbccdd (
	2t7b4g4vsa5smi47k61mv5bv1a22bojr )"
	);
ok( !$rr16->covered('ns1.example'), 'next hashed name not covered (ns1.example.)' );
